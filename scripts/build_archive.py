...
def render_article_page(page: dict) -> str:
    ...
    future_parts = []
    if child_links:
        future_parts.append(f"This branch opens directly into {serial_join_html(child_links)}.")
    if sibling_links:
        future_parts.append(f"Nearby pages in the same branch include {serial_join_html(sibling_links)}.")
    if not future_parts:
