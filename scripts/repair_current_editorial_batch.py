...
def section_prompt(section: Tag) -> str:
    ...
    return strip_prompt_number(prompt_node.get_text(" ", strip=True) if prompt_node else "")
...
def remove_placeholder_list_items(section: Tag) -> bool:
    ...
    for item in section.find_all("li"):
        ...
        if any(pattern.search(text) for pattern in PLACEHOLDER_LIST_PATTERNS):
            item.decompose()
            changed = True
...
def remove_generated_generic_paragraphs(section: Tag) -> bool:
    ...
    for sibling in list(heading.find_next_siblings()):
        ...
        if any(pattern.search(text) for pattern in GENERATED_GENERIC_PATTERNS):
            sibling.decompose()
            changed = True
    return changed
